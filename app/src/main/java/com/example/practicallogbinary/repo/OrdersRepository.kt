...
import com.example.practicallogbinary.utils.ExtFuncs.logI
    ...
    private suspend fun callGetOrdersApi(restaurantId: Int,
        ...
        try {
            ...
            if (apiResponse.isSuccessful) {
                ...
                logD("CheckSomeApiResponse", apiResponse.body().toString())
                if (apiResponse.code() == AppConsts.STATUS_CODE_OK) {
                    try {
                        ...
                        logE("CheckSomeApiException", e.localizedMessage)
                        withContext(Dispatchers.Main) {
                            responseGetter.value = OrdersResponseModel(
                                success = false,
                    ...
                    logD("CheckSomeApiErrorMain", apiResponse.errorBody()?.string() ?: "Error")
                    withContext(Dispatchers.Main) {
                        responseGetter.value = OrdersResponseModel(
                            success = false,
                            message = "Some message"
